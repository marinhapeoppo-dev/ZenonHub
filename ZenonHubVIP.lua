...
-- No Animations Function (FIXED)
local function ToggleNoAnimations(state)
    MartinSC.NoAnim = state
    
    if state then
        -- Method yang work untuk disable animations
...
local function ShowTab(tabName)
    ...
    if tabName == "Fitur" then
        CreatePremiumToggle("No Animations", "Disable character animations", UDim2.new(0, 0, 0, 0), ToggleNoAnimations)
        CreatePremiumToggle("FPS Booster", "Increase game performance", UDim2.new(0, 0, 0, 70), ToggleFPSBoost)
        CreatePremiumToggle("Auto Click", "Automatic mouse clicking", UDim2.new(0, 0, 0, 140), ToggleAutoClick)
        CreateSpeedControl(UDim2.new(0, 0, 0, 210))
        CreatePremiumToggle("Auto Farm", "Auto Lempar & Tarik Ikan", UDim2.new(0, 0, 0, 290), ToggleAutoFarm)
